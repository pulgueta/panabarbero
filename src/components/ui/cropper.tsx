...
function CropperImpl(props: CropperImplProps) {
  ...
  const onSafariZoomPrevent = useCallback(
    (event: Event) => event.preventDefault(),
    [],
  );

  const onEventsCleanup = useCallback(() => {
    // @ts-expect-error - MouseEvent is not iterable
    document.removeEventListener("mousemove", onMouseMove);
    // @ts-expect-error - TouchEvent is not iterable
  ...
  const onWheelZoom = useCallback(
    (event: WheelEvent) => {
      ...
      event.preventDefault();
      // @ts-expect-error - WheelEvent is not iterable
    ...
    (event: TouchEvent<RootElement>) => {
      ...
      if (event.touches.length === 2) {
        const [firstTouch, secondTouch] = event.touches
          ...
          : [];
        if (firstTouch && secondTouch) {
          // @ts-expect-error - Touch is not iterable
          const pointA = getTouchPoint(firstTouch);
          // @ts-expect-error - Touch is not iterable
          const pointB = getTouchPoint(secondTouch);

...
import { IconStack } from "@/components/calendar/icon-stack";
...
function CalendarAgendaView({
  ...
  useCalendarSelector((state) => state.events);
 ...
 */
function CalendarAgendaItem({ segment }: { segment: CalendarSegment }) {
  const viewConfig = useCalendarViewConfig();
  return (
    <CalendarChip
      segment={segment}

...
import { expandRecurrence } from "@/components/calendar/calendar-recurrence";
import type {
  CalendarDateRange,
  CalendarEvent,
  CalendarOccurrence,
...
function rangesIntersect(a: CalendarDateRange, b: CalendarDateRange): boolean {
...
function segmentOccurrence<TData>(
  ...
  const segments: CalendarSegment<TData>[] = [];
 ...
 */
function packTimedSegments<TData>(segments: CalendarSegment<TData>[]): void {
  if (segments.length === 0) return;

  type Working = {
    seg: CalendarSegment<TData>;
    startMin: number;
    effEnd: number;
  };
...
interface CalendarDayBucket<TData = unknown> {
  allDay: CalendarSegment<TData>[];
  timed: CalendarSegment<TData>[];
}

interface CalendarWeekRow<TData = unknown> {
  rowIndex: number;
  rowStart: Date;
...
interface CalendarIndex<TData = unknown> {
  occurrences: CalendarOccurrence<TData>[];
  byDay: Map<string, CalendarDayBucket<TData>>;
  weekRows: CalendarWeekRow<TData>[];
}
...
function defaultOccurrenceOrder(
...
function buildCalendarIndex<TData>(
  ...
  const occurrences: CalendarOccurrence<TData>[] = [];
  for (const event of events) {
  ...
  const byDay = new Map<string, CalendarDayBucket<TData>>();
  const barSegmentsByRow = new Map<number, CalendarSegment<TData>[]>();
  ...
  const weekRows: CalendarWeekRow<TData>[] = [];
...
function getRangeKey(range: CalendarDateRange): string {
...
function flattenResources(
  ...
  depth = 0,
): Array<{ resource: CalendarResource; depth: number }> {
  const rows: Array<{ resource: CalendarResource; depth: number }> = [];
...
function resolveOffDay(
  day: Date,
  timeZone: string,
  config: boolean | CalendarOffDaysConfig | undefined,
  defaultWeekendDays?: number[],
): boolean {
  if (!config) return false;
  const resolved: CalendarOffDaysConfig = config === true ? {} : config;
  const weekendDays =

...
function CalendarNavToday({
  ...
  const { today, isToday } = useCalendarNavigation();
...
function CalendarNavPrev({
  ...
  const { prev } = useCalendarNavigation();
  const settings = useCalendarSettings();
  const nav = useNavButtonProps();
  return (
    <NavTooltip
    ...
    >
      <Button
        ...
        {...props}
      >
        {children ?? <CaretLeftIcon className="size-4" aria-hidden="true" />}
      </Button>
    </NavTooltip>
  );
}
...
function CalendarNavNext({
  ...
  const { next } = useCalendarNavigation();
  const settings = useCalendarSettings();
  const nav = useNavButtonProps();
  return (
    <NavTooltip
    ...
    >
      <Button
        ...
        {...props}
      >
        {children ?? <CaretRightIcon className="size-4" aria-hidden="true" />}
      </Button>
    </NavTooltip>
  );
}

interface CalendarTitleProps extends useRender.ComponentProps<"div"> {
...
function CalendarTitle({
  ...
  const { title } = useCalendarNavigation();
  const viewConfig = useCalendarViewConfig();
  const defaultProps = {
...
function CalendarViewSwitcher({
  ...
  const { view, dayCount, availableViews, setView } = useCalendarView();
  const settings = useCalendarSettings();
  const viewConfig = useCalendarViewConfig();
  const nav = useNavButtonProps();
  const tooltips = useNavTooltipConfig();
              ...
              viewConfig.dayCountPresets.map((count) => (
                ...
                >
                  ...
                  {viewConfig.enableShortcuts && (
                    <CalendarViewShortcut>{count}</CalendarViewShortcut>
                  )}
                </DropdownMenuItem>
              ))
            ) : (
...
function CalendarViewShortcut({ children }: { children: ReactNode }) {
  const viewConfig = useCalendarViewConfig();
  return (
    <kbd
      data-slot="calendar-view-shortcut"
      className={cn(
 ...
 * useCalendarNavigation().goTo). No tooltip by default: it opens an
...
function CalendarDatePicker({
  ...
  const { date, goTo, activeRange } = useCalendarNavigation();
...
type CalendarToolbarProps = useRender.ComponentProps<"div">;
...
interface CalendarNavProps extends useRender.ComponentProps<"div"> {
  /**

 ...
 */
const CALENDAR_COLORS: Array<{ name: string; value: string }> = [
 ...
 *   by the dnd engine, data-slot=calendar-drag-carry) travels with the
 ...
 */
const CALENDAR_GHOST = {
  move: "rounded-sm border border-dashed border-(--cal-event-color)/50 bg-(--cal-event-color)/8",
  resize:
    "rounded-sm border border-dashed border-(--cal-event-color)/70 overflow-hidden",
...
interface CalendarChipContextValue<TData = unknown> {
...
const CalendarChipContext =
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  createContext<CalendarChipContextValue<any> | null>(null);

/** The chip's subject; usable inside renderEvent content and chip children. */
function useCalendarChip<TData = unknown>(): CalendarChipContextValue<TData> {
  const ctx = useContext(CalendarChipContext);
  if (!ctx) {
    throw new Error("useCalendarChip must be used within <CalendarChip>");
  }
  return ctx as CalendarChipContextValue<TData>;
}

interface CalendarChipProps<TData = unknown>
 ...
 */
function CalendarChip<TData = unknown>({
  segment,
  className,
  render,
  children,
  preview = false,
  ...props
}: CalendarChipProps<TData>) {
  const instance = useCalendar<TData>();
  const viewConfig = useCalendarViewConfig<TData>();
  const { view } = useCalendarViewContext();
  const gestures = useCalendarGestures<TData>();
  const { settings } = instance;
  const occurrence = segment.occurrence;
  const event = occurrence.event;

  const isSelectedRaw = useCalendarSelector<TData, boolean>(
    (state) => state.selection.eventKeys.includes(occurrence.key),
    { calendar: instance },
  );
  const isDraggingRaw = useCalendarSelector<TData, boolean>(
  ...
  const resizeOn = useCalendarSelector<TData, boolean>(
  ...
  const defaultContent = (
    <>
      ...
      <span
        className={cn(
          "font-medium",
          stackedBlock ? CALENDAR_FADE_TRUNCATE : "truncate",
        )}
      >
        {event.title}
      </span>
  ...
  const agendaDefaultContent = (
    <>
      ...
      <span
        aria-hidden
        data-slot="calendar-agenda-dot"
        className="size-2 shrink-0 rounded-full bg-(--cal-event-color)"
      />
  ...
  const grip = (
    <span
      aria-hidden
      data-slot="calendar-resize-grip"
      className={cn(
    ...
    <>
      {timedBlock && segment.isStart && (
        <span
          ...
          data-edge="start"
          className={cn(
            "absolute inset-x-1 top-0 flex h-1.5 cursor-ns-resize items-center justify-center opacity-0 transition-opacity duration-150 group-hover/cal-chip:opacity-100",
      ...
      {timedBlock && segment.isEnd && (
        <span
          ...
          data-edge="end"
          className={cn(
            "absolute inset-x-1 bottom-0 flex h-1.5 cursor-ns-resize items-center justify-center opacity-0 transition-opacity duration-150 group-hover/cal-chip:opacity-100",
            viewConfig.classNames?.resizeHandle,
          )}
      ...
      {(horizontalBar || (isBar && inTimeGrid)) && segment.isStart && (
        <span
          ...
          data-edge="start"
          className={cn(
            "absolute inset-y-0 start-0 flex w-2 cursor-ew-resize items-center justify-center opacity-0 transition-opacity duration-150 group-hover/cal-chip:opacity-100",
            viewConfig.classNames?.resizeHandle,
          )}
          onPointerDown={(e) => gestures.beginResize(e, segment, "start")}
        >
      ...
      {(horizontalBar || (isBar && inTimeGrid)) && segment.isEnd && (
        <span
          ...
          data-edge="end"
          className={cn(
            "absolute inset-y-0 end-0 flex w-2 cursor-ew-resize items-center justify-center opacity-0 transition-opacity duration-150 group-hover/cal-chip:opacity-100",
            viewConfig.classNames?.resizeHandle,
          )}
          onPointerDown={(e) => gestures.beginResize(e, segment, "end")}
        >
  ...
  const defaultProps = {
    ...
    style: {
      "--cal-event-color": event.color ?? "var(--color-primary)",
    } as CSSProperties,
    ...
    className: cn(
      "group/cal-chip text-foreground relative flex w-full min-w-0 cursor-pointer touch-none items-center overflow-hidden text-start select-none",
      ...
      view === "agenda"
        ...
        : cn(
            ...
            "bg-(--cal-event-color)/15 hover:bg-(--cal-event-color)/25",
            // a flat tint reads darker over a dark surface, so lift it a little
            // in dark mode to keep a lighter, softer chip tone
            "dark:bg-(--cal-event-color)/20 dark:hover:bg-(--cal-event-color)/30",
            "inset-ring inset-ring-(--cal-event-color)/15",
            "transition-[background-color,box-shadow] duration-150",
            "data-dragging:opacity-40",
            "data-selected:bg-(--cal-event-color)/30 data-selected:inset-ring-(--cal-event-color)/40",
...
export type { CalendarChipContextValue, CalendarChipProps };
export {
  CALENDAR_COLORS,
  CALENDAR_FADE_TRUNCATE,
  CALENDAR_GHOST,
  CalendarChip,
  useCalendarChip,
};

...
const PERIOD_MS: Record<CalendarRecurrenceRule["freq"], number> = {
  daily: 86400000,
  weekly: 604800000,
...
class CalendarRecurrenceError extends Error {
...
function parseRRuleString(
  ...
  const rule: Partial<CalendarRecurrenceRule> = {};
    ...
    switch (key) {
      case "FREQ": {
        ...
        if (
          ...
          throw new CalendarRecurrenceError(`FREQ=${value}`);
        }
        rule.freq = freq;
        break;
      }
      ...
      case "BYDAY":
        rule.byWeekday = value.split(",").map((token) => {
          ...
          if (!match) throw new CalendarRecurrenceError(`BYDAY=${token}`);
          const day = match[2] as CalendarWeekday;
      ...
      case "BYMONTHDAY":
        rule.byMonthDay = value.split(",").map((v) => {
          ...
          if (Number.isNaN(day)) {
            throw new CalendarRecurrenceError(`BYMONTHDAY=${v}`);
          }
          return day;
        });
        break;
      case "BYMONTH":
        ...
        break;
      case "WKST": {
        const day = value.trim().toUpperCase() as CalendarWeekday;
        if (!WEEKDAYS.includes(day)) {
          throw new CalendarRecurrenceError(`WKST=${value}`);
        }
        rule.weekStart = day;
        break;
      }
      default:
        throw new CalendarRecurrenceError(key ?? pair);
    }
  }

  if (!rule.freq) throw new CalendarRecurrenceError("missing FREQ");
  return rule as CalendarRecurrenceRule;
}
...
function parseRRuleDate(value: string, timeZone?: string): Date {
  ...
  if (!match) throw new CalendarRecurrenceError(`UNTIL=${value}`);
  ...
  if (Number.isNaN(date.getTime())) {
    throw new CalendarRecurrenceError(`UNTIL=${value}`);
  }
  return date;
}

/** Serializes the structured subset back to an RRULE line (without prefix). */
function formatRRuleString(rule: CalendarRecurrenceRule): string {
 ...
 * in buildCalendarIndex, where the override event and its parent series meet.
 */
function expandRecurrence<TData>(
  ...
  // still receive chronological order (buildCalendarIndex re-sorts regardless).

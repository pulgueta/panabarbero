...
import type { CalendarInstance } from "@/components/calendar/calendar";
import {
  useCalendar,
...
function cancelActiveCalendarGestures(): void {
...
function collectSurface(
  ...
  const root =
    origin.closest<HTMLElement>("[data-slot=calendar-time-grid]") ??
    origin.closest<HTMLElement>("[data-slot=calendar-resource-view]") ??
    origin.closest<HTMLElement>("[data-slot=calendar-month-view]") ??
    origin.closest<HTMLElement>("[data-slot=calendar]") ??
    fallbackRoot ??
    null;
  ...
  if (root) {
    for (const el of root.querySelectorAll<HTMLElement>("[data-cal-day]")) {
      const day = new Date(Number(el.dataset.calDay));
      if (el.dataset.calBoundsStart !== undefined) {
        columns.push({
          day,
          rect: el.getBoundingClientRect(),
          boundsStartMin: Number(el.dataset.calBoundsStart),
          boundsEndMin: Number(el.dataset.calBoundsEnd),
          resourceId: el.dataset.calResource,
        });
      } else {
...
function beginBlockedGesture<TData>(
  ...
  const onMove = (e: PointerEvent) => {
    ...
    // detect the blocked-drag state (mirrors "cal-dragging")
    document.body.classList.add("cal-drag-blocked");
  ...
  const cleanup = () => {
    ...
    if (activated) {
      ...
      document.body.classList.remove("cal-drag-blocked");
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  const activation = { ...CALENDAR_ACTIVATION, ...settings.activation };
  const startX = startEvent.clientX;
  const startY = startEvent.clientY;
  const pointerId = startEvent.pointerId;
  ...
  const announcer =
    origin
      ...
      ?.querySelector<HTMLElement>("[data-slot=calendar-announcer]") ??
    internals
      .getRootEl()
      ?.querySelector<HTMLElement>("[data-slot=calendar-announcer]") ??
    null;
  ...
  const setBodyDragging = (on: boolean, invalid = false) => {
    document.body.classList.toggle("cal-dragging", on);
  ...
  const applyProposal = (e: PointerEvent) => {
    ...
    const update: CalendarProposedUpdate<TData> = {
  ...
  // drop slot (CALENDAR_GHOST.move). Vanilla DOM: cloned once at
  ...
  const createCarry = () => {
    if (kind !== "move") return;
    const chip =
      origin.closest<HTMLElement>("[data-slot=calendar-chip]") ?? origin;
    ...
    carryEl.setAttribute("data-slot", "calendar-drag-carry");
  ...
  const updateHint = (e: PointerEvent) => {
    ...
    if (!hintEl) {
      ...
      hintEl.setAttribute("data-slot", "calendar-drop-hint");
...
function useCalendarGestures<TData = unknown>() {
  ...
  const { classNames } = useCalendarViewConfig<TData>();
  const ui = useMemo(
    () => ({
  ...
  useEffect(() => {
    ...
    return () => {
      gestureConsumers -= 1;
      if (gestureConsumers === 0) cancelActiveCalendarGestures();
    };
  }, []);
  ...
  const beginMove = useCallback(
    (e: React.PointerEvent, segment: CalendarSegment<TData>) => {
